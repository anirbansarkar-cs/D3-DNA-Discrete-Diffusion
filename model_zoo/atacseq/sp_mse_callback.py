"""
ATACSeq-specific SP-MSE Validation Callback

This module implements the SP-MSE validation callback specifically for the ATACSeq dataset,
inheriting from the base callback and providing ATACSeq-specific oracle loading and prediction.
"""
...
# import atacseq oracle still to avoid bugs with dependencies - won't be used
from model_zoo.deepstarr.deepstarr import PL_DeepSTARR


class ATACSeqSPMSECallback(BaseSPMSEValidationCallback):
    """SP-MSE validation callback specifically for ATACSeq dataset."""
    
    def get_default_sampling_steps(self) -> int:
        """Get default sampling steps for ATACSeq (sequence length)."""
        return 249
    ...
    def load_oracle_model(self):
        ...
        try:
            ...
            print(f"Failed to load ATACSeq (DeepSTARR) oracle model: {e}")
            return None
    
    def get_oracle_predictions(self, sequences: torch.Tensor, device: torch.device) -> torch.Tensor:
        """
        Get ATACSeq oracle predictions for sequences.
        ...
        # ATACSeq expects input as (batch_size, channels, length)
...
def create_atacseq_sp_mse_callback(cfg, dataset_name: str = 'atacseq'):
    """
    Create ATACSeq SP-MSE callback from configuration.
    
    Args:
        cfg: Configuration object
        dataset_name: Dataset name (default: 'atacseq')
        
    Returns:
        ATACSeqSPMSECallback instance or None if not enabled
    """
    ...
    if oracle_path is None:
        oracle_path = 'model_zoo/atacseq/oracle_models/oracle_ATACSeq_ATACSeq_data.ckpt'
    
    data_path = sp_mse_cfg.get('data_path')
    if data_path is None:
        data_path = 'model_zoo/atacseq/ATACSeq_data.h5'
    
    return ATACSeqSPMSECallback(
