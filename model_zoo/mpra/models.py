...
def load_trained_model(checkpoint_path: str, config: DictConfig, architecture: str, device: str = 'cuda') -> Tuple[torch.nn.Module, Any, Any]:
    """
    Load trained MPRA model from specific checkpoint file.
    ...
    print(f"Loading MPRA {architecture} model from {checkpoint_path}")
    ...
    # Create model
    if architecture.lower() == 'transformer':
        model = MPRATransformerModel(config)
    elif architecture.lower() == 'convolutional':
        model = MPRAConvolutionalModel(config)
    else:
    ...
    print(f"✓ MPRA {architecture} model loaded successfully")
