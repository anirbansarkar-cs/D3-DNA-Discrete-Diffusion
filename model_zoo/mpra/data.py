...
class MPRADataset(Dataset):
    ...
    def __init__(self, h5_file_path: str, split: str = 'train'):
        ...
        if not os.path.exists(h5_file_path):
            raise FileNotFoundError(f"MPRA data file not found: {h5_file_path}")
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with h5py.File(self.h5_file_path, 'r') as dataset_file:
            # Determine which split to load
            if self.split == 'train':
                x = torch.tensor(np.array(dataset_file['x_train']).astype(np.float32))
                y = torch.tensor(np.array(dataset_file['y_train']).astype(np.float32))
            elif self.split == 'valid':
                x = torch.tensor(np.array(dataset_file['x_valid']).astype(np.float32))
                y = torch.tensor(np.array(dataset_file['y_valid']).astype(np.float32))
            elif self.split == 'test':
                x = torch.tensor(np.array(dataset_file['x_test']).astype(np.float32))
                y = torch.tensor(np.array(dataset_file['y_test']).astype(np.float32))
            else:
...
def get_mpra_dataloaders(config, distributed: bool = True) -> Tuple[DataLoader, DataLoader]:
    """
    Get MPRA dataloaders for training and validation.
...
def get_mpra_dataloaders_with_cycle(config, distributed: bool = True) -> Tuple[DataLoader, DataLoader]:
    """
    Get MPRA dataloaders with cycle_loader applied for training and validation.
    ...
    """
    train_loader, valid_loader = get_mpra_dataloaders(config, distributed)
